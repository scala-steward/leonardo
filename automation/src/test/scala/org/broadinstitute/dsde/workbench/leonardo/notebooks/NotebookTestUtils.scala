...
trait NotebookTestUtils extends LeonardoTestUtils {
  ...
  def withNotebooksListPage[T](cluster: ClusterProjectAndName)(testCode: NotebooksListPage => T)(implicit webDriver: WebDriver, token: AuthToken): T = {
  ...
  def withFileUpload[T](cluster: Cluster, file: File)(testCode: NotebooksListPage => T)(implicit webDriver: WebDriver, token: AuthToken): T = {
    withNotebooksListPage(ClusterProjectAndName(cluster.googleProject, cluster.clusterName)) { notebooksListPage =>
  ...
  def withNewNotebook[T](cluster: ClusterProjectAndName, kernel: NotebookKernel = Python2, timeout: FiniteDuration = 2.minutes)(testCode: NotebookPage => T)(implicit webDriver: WebDriver, token: AuthToken): T = {
  ...
  def withLocalizeDelocalizeFiles[T](cluster: ClusterProjectAndName, fileToLocalize: String, fileToLocalizeContents: String,
  ...
  def verifyLocalizeDelocalize(cluster: ClusterProjectAndName, localizedFileName: String, localizedFileContents: String,
  ...
  def verifyAndSaveLocalizationLog(cluster: ClusterProjectAndName)(implicit token: AuthToken): File = {
