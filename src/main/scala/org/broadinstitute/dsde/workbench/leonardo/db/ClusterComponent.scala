...
trait ClusterComponent extends LeoComponent {
  ...
  object clusterQuery extends TableQuery(new ClusterTable(_)) {
    ...
    def save(cluster: Cluster, initBucket: GcsBucketName, serviceAccountKeyId: Option[ServiceAccountKeyId]): DBIO[Cluster] = {
      (clusterQuery returning clusterQuery.map(_.id) += marshalCluster(cluster, GcsPath(initBucket, GcsObjectName("")).toUri, serviceAccountKeyId)) flatMap { clusterId =>
    ...
    private def unmarshalCluster(clusterRecord: ClusterRecord, labels: LabelMap): Cluster = {
      ...
      Cluster(
        ...
        serviceAccountInfo,
        machineConfig,
        Cluster.getClusterUrl("todo", project, name),
