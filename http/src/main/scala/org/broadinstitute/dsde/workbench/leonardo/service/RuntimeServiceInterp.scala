...
class RuntimeServiceInterp[F[_]: Parallel](config: RuntimeServiceConfig,
  ...
  override def createRuntime(
    ...
    for {
      context <- as.ask
      hasPermission <- authProvider.hasProjectPermission(SamResource.Project(googleProject), userInfo, CreateClusters)
      ...
      _ <- clusterOpt match {
        ...
        case None =>
          for {
            samResource <- F.delay(SamResource.Runtime(UUID.randomUUID().toString))
            ...
            _ <- authProvider
              .notifyClusterCreated(samResource, userInfo.userEmail, googleProject, runtimeName)
              .handleErrorWith { t =>
                log.error(t)(
  ...
  override def getRuntime(userInfo: UserInfo, googleProject: GoogleProject, runtimeName: RuntimeName)(
    ...
    for {
      ...
      hasPermission <- authProvider.hasNotebookClusterPermission(resp.samResource,
                                                                 userInfo,
                                                                 GetClusterStatus,
                                                                 googleProject,
                                                                 runtimeName)
  ...
  override def listRuntimes(userInfo: UserInfo, googleProject: Option[GoogleProject], params: Map[String, String])(
    ...
    for {
      ...
      samVisibleClusters <- authProvider
        .filterUserVisibleClusters(userInfo, clusters.map(c => (c.googleProject, c.samResource)))
    } yield {
  ...
  override def deleteRuntime(userInfo: UserInfo, googleProject: GoogleProject, runtimeName: RuntimeName)(
    ...
    for {
      ...
      hasPermission <- authProvider.hasNotebookClusterPermission(runtime.samResource,
                                                                 userInfo,
                                                                 GetClusterStatus,
                                                                 googleProject,
                                                                 runtimeName)
